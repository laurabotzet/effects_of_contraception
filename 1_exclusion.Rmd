...
* participants, who either ‘took a chance’ on getting pregnant or used no contraception for ‘other reasons’
...
all_surveys = all_surveys %>% 
  mutate(reasons_for_exclusion = str_c(reasons_for_exclusion,
                                       if_else(gender != 1 |, "not_female, ", "")))

table(all_surveys$reasons_for_exclusion %contains% "not_female")
```
...
table(all_surveys$reasons_for_exclusion %contains% "didnt_finish_initial")
```
...
table(all_surveys$reasons_for_exclusion %contains% "menopausal")
```
...
table(all_surveys$reasons_for_exclusion %contains% "older_than_50")
```
...
table(all_surveys$reasons_for_exclusion %contains% "pregnant")
```
...
table(all_surveys$reasons_for_exclusion %contains% "breast_feeding")
```
...
table(all_surveys$reasons_for_exclusion_contraception %contains% "contraception_barrier_no_penetrative_sex")
```
...
table(all_surveys$reasons_for_exclusion_contraception %contains% "contraception_hormonal_morning_after_pill")
```
...
table(all_surveys$reasons_for_exclusion_contraception %contains% "contraception_breast_feeding")
```
...
table(all_surveys$reasons_for_exclusion_contraception %contains% "contraception_infertile")
```
...
table(all_surveys$reasons_for_exclusion_contraception %contains% "contraception_partner_infertile")
```
...
table(all_surveys$reasons_for_exclusion_contraception %contains% "contraception_sterilised")
```
...
table(all_surveys$reasons_for_exclusion_contraception %contains% "contraception_partner_sterilised")
```
...
table(all_surveys$reasons_for_exclusion_contraception %contains% "contraception_not_listed")
```
...
table(all_surveys$reasons_for_exclusion %contains% "contraceptive_method")
```
...
table(all_surveys$reasons_for_exclusion %contains% "not_heterosexual_female")
```
...
table(all_surveys$reasons_for_exclusion %contains% "pregnant_trying")
```
...
table(all_surveys$reasons_for_exclusion %contains% "pregnant_chance")
```
...
table(all_surveys$reasons_for_exclusion %contains% "no_contraception")
```
...
table(all_surveys$reasons_for_exclusion %contains% "incongruent_information_about_hormonal_contraception")
```
...
table(all_surveys$reasons_for_exclusion %contains% "sex_hormones")
```

## Total number of excluded participants
```{r}
table(all_surveys$reasons_for_exclusion == "")
```
In total 497 participants were excluded, leaving 1163 subjects which were included in the analysis.
...
Lab participants that did not do the online diary
...
Cycle longer than 50 days.
```{r}
diary = diary %>% 
  mutate(reasons_for_exclusion = str_c(reasons_for_exclusion,
                                            if_else(coalesce(minimum_cycle_length_diary, as.numeric(menstruation_length)) > 50, "cycle_longer_than_50, ", "", "")
                                       )
  )
```
...
diary = diary %>% 
  mutate(reasons_for_exclusion = str_c(reasons_for_exclusion,
                                            if_else(is.na(fertile_fab), "fertility_unknown, ", "")
                                       )
  )
```
...
diary = s1_filter %>% full_join(diary, by = c('session', 'short'), suffix = c("_filter", ""))
...
save(diary_social, diary, sex_long, network_nominations, network, s1_demo, s1_filter, s2_initial, s3_daily, s4_followup, s4_timespent, withfollowup, all_surveys, file = "cleaned_selected.rdata")
``` 
...
crosstabs(~ included_d + (reasons_for_exclusion == ""), diary)
