...
ggplot(data, aes(relationship_duration)) +
  geom_histogram(aes(fill = relationship_duration_factor2), bins = 100)

m1 = lm(bfi_extra ~ relationship_duration, data = data)
m2 = lm(bfi_extra ~ relationship_duration_factor, data = data)
m3 = lm(bfi_extra ~ relationship_duration_factor2, data = data)
...
datax = data %>% mutate(
    ...
    contraception_approach = if_else(
        ...
        true = if_else(
          condition =
            com == "hormonal_pill" | com == "hormonal_other" |
        ...
        if_else(
          ...
          true = if_else(condition =
                           com != "",
                         true = if_else(condition =
                                          com %contains% "barrier_intrauterine_pessar",
                                        true = "barrier_pessar",
                                        if_else(condition =
                                                  com %contains% "barrier_condoms",
                                                true = "condoms",
                                                false = "other")),
                         false = "nothing"),
          false = "awareness")))
